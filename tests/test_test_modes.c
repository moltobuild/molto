...
MOLTEST(per_file_mode_is_still_the_default) {
    ...
    str_list binaries;
    str_list_init(&binaries);
    EXPECT_TRUE(build_tests(root, profile_debug, false, &binaries, NULL) != exit_ok);
...
MOLTEST(an_unknown_test_mode_is_a_manifest_error) {
    ...
    str_list binaries;
    str_list_init(&binaries);
    EXPECT_TRUE(build_tests(root, profile_debug, false, &binaries, NULL) == exit_invalid_manifest);
