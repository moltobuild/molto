...
static bool compose_features(const project_target *target, char *out, size_t out_size) {
    ...
    for (size_t i = 0; i < target->requires_count; i++) {
        int written = snprintf(out + used, out_size - used, "%s%s",
                               i > 0 ? FEATURE_SEPARATOR : "", target->requires[i]);
...
static bool compose_request(const project_target *target, bool needs_cpp,
...
static bool build_pickup_argv(const char *program, const project_target *target,
                              bool needs_cpp, const char *features, str_list *argv) {
    bool ok = str_list_push(argv, program)
           && str_list_push(argv, ARG_RESOLVE)
           && str_list_push(argv, ARG_LANG)
           && str_list_push(argv, needs_cpp ? LANG_CXX : LANG_C);
    ...
    return ok
        && str_list_push(argv, ARG_FORMAT)
        && str_list_push(argv, FORMAT_TOML);
}
...
static bool parse_answer(const char *toml, resolved_toolchain *out) {
    ...
    (void)toml_get_string(doc, ANSWER_SECTION, ANSWER_VENDOR,
                          out->vendor, sizeof out->vendor);
    (void)toml_get_string(doc, ANSWER_SECTION, ANSWER_VERSION,
...
static bool take_from_wsdb(wsdb *db, const char *request, resolved_toolchain *out) {
    ...
    if (ok) {
        ...
        ok = fs_format_path(out->cc, sizeof out->cc, "%s", str_list_get(&values, 0))
          ...
          && fs_format_path(out->version, sizeof out->version, "%s",
                            str_list_get(&values, 3));
    }
...
static void remember(wsdb *db, const char *request, const resolved_toolchain *chain) {
    ...
    bool ok = str_list_push(&values, chain->cc)
           && str_list_push(&values, chain->cxx)
           && str_list_push(&values, chain->vendor)
           && str_list_push(&values, chain->version);
...
static int ask_pickup(const project_target *target, bool needs_cpp,
...
int toolchain_resolve(const project_target *target, bool needs_cpp, wsdb *db,
